        ...
        return $table
            ...
            ->actions([
                ...
                Tables\Actions\Action::make('dep_approve')
                    ...
                    ->color('success')
                    ->visible(fn ($record) => $record->status->value === PettyCashStatus::Submitted->value && $record->user->department->user->id == Auth::user()->id)
                    ->action(function ($record) {
                        $record->update([
                ...
                Tables\Actions\Action::make('dep_reject')
                    ...
                    ->color('danger')
                    ->visible(fn ($record) => $record->status->value === PettyCashStatus::Submitted->value && $record->user->department->user->id == Auth::user()->department->hod)
                    ->action(function ($record) {
                        $record->update([
                ...
                Tables\Actions\Action::make('add_pv')
                    ...
                    ->icon('heroicon-o-check-circle')
                    ->color('success')
                    ->visible(fn (PettyCashReimbursment $record) => in_array($record->status->value, [
                        PettyCashStatus::DepApproved->value,
                        PettyCashStatus::FinApproved->value,
                    ], true)
                        && Auth::user()->can('pv_approve_petty::cash::reimbursment')
                        && ! $record->hasPvNumbers())
                    ->form([
                    ...
                    ->action(function (PettyCashReimbursment $record, array $data) {
                        $record->update([
                ...
                Tables\Actions\Action::make('fin_reject')
                    ...
                    ->requiresConfirmation()
                    ->icon('heroicon-o-x-circle')
                    ->visible(fn (PettyCashReimbursment $record) => $record->status->value === PettyCashStatus::DepApproved->value
                        && $record->hasPvNumbers()
                        && Auth::user()->can('fin_hod_approve_petty::cash::reimbursment'))
