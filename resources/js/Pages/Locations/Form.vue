...
const submit = () => {
    isEdit
        ? form.put(route('app.locations.update', props.record.id))
        : form.post(route('app.locations.store'));
};
</script>

<template>
    <AppLayout>
        <template #header>{{ isEdit ? 'Edit location' : 'Create location' }}</template>
        <form class="mx-auto max-w-xl space-y-4 rounded-xl border border-slate-200 bg-white p-6 shadow-sm" @submit.prevent="submit">
            ...
            <div class="flex justify-between pt-2">
                <Link :href="route('app.locations.index')" class="text-sm text-slate-500">Cancel</Link>
